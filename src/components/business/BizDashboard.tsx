...
export default function BizDashboard() {
  ...
  const [recentTxs, setRecentTxs] = useState(RECENT_BIZ_TRANSACTIONS);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const amt = +(5 + Math.random() * 25).toFixed(2);
      const newEntry = {
        id: `bt_${Date.now()}`,
        fanFlag: fan.flag,
        fanCountry: fan.country,
        amount: amt,
        points: Math.round(amt * 2),
        time: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
        items: 'Live order',
      };
      setRevenue(r => +(r + amt).toFixed(2));
      setTxCount(c => c + 1);
      setNewTx(true);
      setRecentTxs(prev => [newEntry, ...prev.slice(0, 4)]);
      setTimeout(() => setNewTx(false), 2000);
    }, 8000);
        ...
        <div className="rounded-3xl p-5 relative overflow-hidden mb-4"
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-2xl font-black text-white">${revenue.toFixed(2)}</p>
              <p className="text-green-300 text-xs">USDC</p>
            </div>
            <div>
              <p className="text-green-200 text-xs mb-1">Transactions</p>
              <p className="text-2xl font-black text-white">{txCount}</p>
              <p className="text-green-300 text-xs">payments</p>
            </div>
            <div>
              <p className="text-green-200 text-xs mb-1">Avg. Ticket</p>
              <p className="text-2xl font-black text-white">${(revenue / txCount).toFixed(2)}</p>
              <p className="text-green-300 text-xs">per visit</p>
            </div>
          </div>
        </div>
