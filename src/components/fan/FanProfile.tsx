...
export default function FanProfile() {
  ...
  const walletCard = (
    <div className="glass-card rounded-2xl p-4 border border-gray-700 mb-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="text-xs text-gray-400 bg-gray-800 px-3 py-1 rounded-lg active:bg-gray-700 transition-all"
    ...
    <div className="glass-card rounded-2xl p-4 border border-gray-700 mb-4">
      ...
      <div className="bg-gray-800 rounded-full h-2.5 mb-2">
        <div className="h-2.5 rounded-full transition-all"
             style={{ width: `${pct}%`, background: pct > 80 ? 'linear-gradient(90deg, #FF6B6B, #FF8E53)' : 'linear-gradient(90deg, #00A651, #FFD700)' }} />
      </div>
      <p className="text-xs text-gray-500">${(budget - spent).toFixed(2)} remaining</p>
      <div className="flex gap-2 mt-3">
        {[300, 500, 1000].map(b => (
          <button key={b} onClick={() => setBudget(b)}
            className={`flex-1 py-1 rounded-xl text-xs font-bold transition-all ${budget === b ? 'bg-brand-green text-white' : 'glass-card text-gray-400 border border-gray-700'}`}>
            ${b}
          </button>
        ))}
      </div>
  ...
  const pointsCard = (
    <div className="glass-card rounded-2xl p-4 border border-yellow-500/20 mb-4"
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 40px 60px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 24 }}>
            <div>
              ...
              {budgetCard}
              {pointsCard}
              <button onClick={() => setRole('picker')}
                className="w-full mt-2 py-3 rounded-2xl border border-gray-700 text-gray-400 font-semibold glass-card text-sm">
                ← Switch Role
              </button>
            </div>
            <div className="glass-card rounded-2xl border border-gray-700 p-5" style={{ alignSelf: 'start' }}>
              {txHistory}
            </div>
          </div>
        </div>
