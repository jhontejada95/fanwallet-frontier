...
const BalanceCard = ({ goalPoints, chainLoading, walletConnected, refreshBalances }: {
  ...
  <div style={{ position: 'relative', background: '#131826', border: '1px solid rgba(255,215,0,0.25)', borderRadius: 24, padding: '24px 22px 22px', overflow: 'hidden', boxShadow: '0 20px 60px -20px rgba(255,215,0,0.4)' }}>
    ...
    <div style={{ fontFamily: 'Archivo, sans-serif', fontWeight: 700, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase' as const, color: '#FFD700', opacity: 0.85 }}>YOUR BALANCE</div>
    {chainLoading ? (
...
const TabBar = ({ tab, setTab }: { tab: string; setTab: (t: 'earn' | 'redeem' | 'history') => void }) => (
  <div style={{ display: 'flex', gap: 4, padding: 4, background: '#131826', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14 }}>
    {(['earn', 'redeem', 'history'] as const).map(t => (
      <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: '9px 0', textAlign: 'center' as const, borderRadius: 11, background: tab === t ? '#FFD700' : 'transparent', color: tab === t ? '#1a1300' : '#B6BECB', fontFamily: 'Archivo, sans-serif', fontWeight: 800, fontSize: 12, textTransform: 'capitalize' as const, border: 'none', cursor: 'pointer', transition: 'all 0.15s' }}>
        {t}
      </button>
    ))}
  </div>
    ...
    <>
      {tab === 'earn' && EARN_WAYS.map((it, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px', background: '#131826', borderRadius: 16, border: '1px solid rgba(255,255,255,0.04)', marginBottom: 8, cursor: 'pointer' }}>
          <div style={{ width: 42, height: 42, borderRadius: 13, background: 'rgba(255,255,255,0.04)', display: 'grid', placeItems: 'center', fontSize: 20, flexShrink: 0 }}>{it.emoji}</div>
          <div style={{ flex: 1 }}>
      ...
      {tab === 'redeem' && (
        <div>
          {txError && <div style={{ fontSize: 12, color: '#EF4444', background: 'rgba(239,68,68,0.1)', borderRadius: 12, padding: '10px 12px', marginBottom: 12 }}>⚠ {txError}</div>}
          {redeemed && (
            <div style={{ fontSize: 12, color: '#00A651', background: 'rgba(0,166,81,0.1)', borderRadius: 12, padding: '10px 12px', marginBottom: 12, textAlign: 'center' as const }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? 'repeat(4,1fr)' : '1fr 1fr', gap: 10 }}>
            {REDEEM_OPTIONS.map(r => (
              <div key={r.pts} style={{ background: '#131826', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 12, opacity: goalPoints >= r.pts ? 1 : 0.4 }}>
                ...
                <button onClick={() => handleRedeem(r.pts)} disabled={goalPoints < r.pts || redeeming}
          ...
          {!walletConnected && <div style={{ fontSize: 12, color: '#6b7280', textAlign: 'center' as const, marginTop: 16 }}>Connect wallet to burn GoalPoints on-chain</div>}
        </div>
      )}
      ...
      <div style={{ minHeight: '100vh', background: '#0a0e1a' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 40px 60px' }}>
          <div style={{ marginBottom: 28 }}>
            <div style={{ fontFamily: '"Archivo Black", sans-serif', fontSize: 28, color: '#fff', letterSpacing: '-0.02em' }}>GoalPoints</div>
            <div style={{ color: '#6b7280', fontSize: 14, marginTop: 4 }}>Earn rewards with every purchase · Redeem for real-world prizes</div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 24 }}>
            ...
            {/* Right: tab content */}
            <div style={{ background: '#0d1120', borderRadius: 20, border: '1px solid rgba(255,255,255,0.05)', padding: '20px 22px', minHeight: 400 }}>
              {tabContent}
            </div>
          </div>
        </div>
    ...
    <div style={{ minHeight: '100vh', background: '#0a0e1a', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 22px 8px' }}>
        <div style={{ width: 36, height: 36, borderRadius: 12, background: '#131826', display: 'grid', placeItems: 'center', border: '1px solid rgba(255,255,255,0.06)' }}>
          <svg viewBox="0 0 24 24" width={20} height={20} fill="none" stroke="currentColor" strokeWidth={1.75} strokeLinecap="round" strokeLinejoin="round"><path d="M15 6l-6 6 6 6"/></svg>
        </div>
